        ...
        <div class="main ms-5 ps-4">
            <nav class="navbar navbar-expand-lg bg-light border border-bottom">
                <div class="container-fluid justify-content-center" style="transform: translate(0px, 10px);">
                    <p class="text-success lead">Rejected Users</p>
                </div>
            </nav>
                        ...
                        <div class="card shadow-sm">
                            <div class="card-body">
                                <div class="d-flex justify-content-between align-items-center">
                                    <div>
                                        <p class="text-muted mb-1">Rejected Users</p>
                                        <h3 class="mb-0 fw-bold" id="totalRejected">0</h3>
                                    </div>
...
<script>
    ...
    async function loadDashboardStats() { // LOAD DASHBOARD CARDS
        ...
        try {
            ...
            document.getElementById("totalRejected").textContent = data.totalPendingRegistrations;
                ...
                return `
                        <tr>
                            <td>
                                ...
                                <td>${data.Program}</td>
                             <td>
                                <button type="button" class="btn text-success" onclick="ReAddUser(${data.student_id})"><i class="bi bi-check2-circle h5"></i></button>
    ...
    function ReAddUser(user_id) { // VERIFICATION CONFIRMATION
        Swal.fire({
            title: "Are you sure?",
            text: `Remove User from the Archives?`,
            icon: "warning",
    ...
    async function verifyUser(user_id) { // SEND VERIFY REQUEST
        ...
        try {
            ...
            const res = await fetch(`../../../api/DeniedStudentsController.php`, {
                method: 'POST',
                body: new URLSearchParams({
                    student_id: user_id,
                    action: 'reEnroll',
